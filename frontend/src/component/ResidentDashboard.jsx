...
function ResidentDashboard() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 dark:bg-slate-950 dark:text-slate-100">
      <header className="sticky top-0 z-20 border-b border-slate-200/80 bg-white/90 backdrop-blur dark:border-slate-800/80 dark:bg-slate-900/90">
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-3 px-4 py-3 sm:px-5 sm:py-4">
          <div className="flex min-w-0 items-center gap-3"><img src={logo} alt="Wecare.sg logo" className="h-10 w-10 shrink-0 rounded-2xl object-contain shadow-sm" /><div className="min-w-0"><h1 className="truncate text-lg font-bold sm:text-xl">wecare.sg</h1><p className="hidden text-xs text-slate-500 dark:text-slate-400 sm:block">Your private wellbeing space</p></div></div>
          <div className="flex shrink-0 items-center gap-1.5"><ThemeToggle /><button onClick={logout} className="rounded-xl px-2.5 py-2 text-xs font-semibold text-slate-600 transition hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-800 sm:px-3 sm:text-sm"><span className="sm:hidden">Exit</span><span className="hidden sm:inline">Log out</span></button></div>
        </div>
      </header>
